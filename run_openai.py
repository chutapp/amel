"""Run AMEL experiment against OpenAI API models."""
...
OPENAI_CONCURRENCY = 20  # OpenAI can handle high concurrency
    ...
    async with semaphore:
        ...
        try:
            ...
            if e.response.status_code == 429:
                ...
                # Retry (semaphore already released since we're outside `async with`)
...
async def run_single(
    ...
    """Run a single condition against OpenAI."""
...
async def run_openai_experiment():
    if not OPENAI_API_KEY:
        print("ERROR: OPENAI_API_KEY not set")
        sys.exit(1)

    output_dir = Path("data/openai")
    ...
    # Save metadata
    metadata = {
        "experiment": "context_bias_openai",
    ...
    async with httpx.AsyncClient() as client:
        # Process in batches
        batch_size = OPENAI_CONCURRENCY * 2
    ...
    print(f"\nOpenAI experiment complete in {total_elapsed/60:.1f} minutes!")
...
if __name__ == "__main__":
    asyncio.run(run_openai_experiment())
