...
Our main findings:
\begin{enumerate}
    \item AMEL appears across all four provider families tested. Across 11 models, conversation history pulls responses toward its prevailing polarity ($d = -0.17$, $p < 10^{-46}$); 9 of 11 models show a significant effect after Bonferroni correction.
    ...
    \item Negative history biases more than positive history: paired per-item $|BS|$ ratio $1.62\times$ ($t = 13.46$, $p < 10^{-39}$, $n = 2{,}481$). Marginal means give $\approx 2\times$ but mix item composition across cells; the paired test is the cleaner statistic.
...
\begin{figure*}[t]
    ...
    \caption{Overview of AMEL. (a) By author-coded item category, ambiguous items absorb the most bias ($d = -0.28$); clear negatives barely budge ($d = +0.03$, n.s.). The same pattern, sharper, when items are stratified by empirical baseline entropy instead: high-entropy items show $d = -0.34$, deterministic-baseline items $d = -0.15$ (Section~\ref{sec:empirical_entropy}). (b) Negative context biases models more than positive context (paired per-item ratio $1.62\times$, $p < 10^{-39}$); marginal means yield $\approx 2\times$ (Section~\ref{sec:polarity}). Even balanced history shifts models toward ``no.'' (c) Bias saturates immediately; 5 turns produce the same effect as 50.}
    \label{fig:hero}
\end{figure*}
...
We complement these mechanistic accounts with a large-scale empirical study: 11 models, 3 domains, systematic variation of context length and polarity.
...
Item categorization was determined by the author with independent verification.
...
We test 11 models from 4 providers, covering a range of scales (Table~\ref{tab:models}).
...
With 7,631 bias score observations, the marginal one-sample $t$-test treating each observation as independent has $>$99\% power to detect $d = 0.10$ at $\alpha = 0.05/21$.
...
The mixed-effects model in Section~\ref{sec:mixed} estimates an ICC of $0.031$ between models; using the Kish design effect $1 + (m-1)\,\text{ICC}$ with the average cluster size $m = 693$ per model gives an effective $n_\text{eff} \approx 7{,}631 / 22.5 \approx 339$.
...
We collected 75,898 API responses (78,084 raw responses; 2,186 duplicate-condition rows from a concurrent-resume bug on the Qwen3 30B run were deduplicated, keeping the first occurrence per condition; see Appendix~\ref{app:dedup}).
After dropping unparseable outputs (7.85\%, concentrated in Claude Opus 4.6 which produces verbose conditional responses that resist binary extraction), 7,631 bias score observations remain.
The unparseable rate differs across conditions ($\chi^2 = 97.7$, $p < 10^{-20}$, $df = 3$): baseline 10.8\%, no-saturated 7.0\%, yes-saturated 8.2\%, neutral 7.7\%.
...
\midrule
Overall effect & $-0.17$ & $<10^{-46}$ & $\bar{BS} = -0.052$ \\
High-entropy items & $-0.34$ & $<10^{-9}$ & Hardest hit \\
Negativity asymmetry & --- & $<10^{-39}$ & paired $1.62\times$ \\
...
The overall bias score is $\bar{BS} = -0.052$ (95\% CI: $[-0.059, -0.045]$; $t(7630) = -14.69$, $p < 10^{-46}$, $d = -0.17$).
...
Figure~\ref{fig:model_comparison} shows susceptibility across all 11 models.
Nine out of eleven show significant bias after Bonferroni correction.
...
Author-coded ambiguous items take the biggest hit: $\bar{BS} = -0.109$, $d = -0.28$, $p < 10^{-42}$.
Clear positives show a moderate effect: $\bar{BS} = -0.052$, $d = -0.16$, $p < 10^{-13}$.
Clear negatives do not move: $\bar{BS} = +0.005$, $d = +0.03$, $p_\text{corrected} = 1.00$.
...
No-saturated contexts shift models by $\bar{BS} = -0.107$ ($d = -0.32$).
Yes-saturated contexts shift models by $\bar{BS} = +0.049$ ($d = +0.19$).
Comparing marginal means gives a ratio of $\approx 2.2\times$, but this confounds item composition between the two cells.
The cleaner statistic is a paired per-item comparison: mean $|BS_\text{no}| = 0.166$ vs.\ $|BS_\text{yes}| = 0.102$, ratio $1.62\times$ ($t = 13.46$, $p < 10^{-39}$, $n = 2{,}481$ pairs).
...
\begin{figure}[t]
    ...
    \caption{Polarity asymmetry. Negative context induces stronger bias than positive context (paired per-item ratio $1.62\times$, $p < 10^{-39}$; marginal means give $\approx 2.2\times$). Even balanced (50/50) history produces a negative shift. Error bars: 95\% CIs.}
    \label{fig:polarity}
\end{figure}
...
The between-model variance is small (ICC $= 0.031$): 9 of 11 models show a significant effect after Bonferroni correction, and the model-to-model differences in magnitude are modest relative to the within-model variation.
...
This affects one of eleven models and does not change overall conclusions, but the dedup procedure is one more place where reviewer attention is warranted.
...
\paragraph{Item categorization by single author.}
The clear/ambiguous/clear-negative labels were assigned by the author with intra-rater agreement (95\%) but without inter-rater validation.
The empirical-entropy stratification in Section~\ref{sec:empirical_entropy} largely bypasses this concern by using the model's own baseline behavior to group items, and the empirical-entropy result is the more robust version of the ``uncertainty absorbs bias'' finding.
An independent inter-rater study would still strengthen the author-coded labels used elsewhere in the paper.

\paragraph{Differential unparseable rates across conditions.}
The 7.85\% unparseable rate is not independent of polarity (Appendix~\ref{app:unparseable}): baseline 10.8\%, no-saturated 7.0\%, yes-saturated 8.2\%, neutral 7.7\%.
...
Three follow-up studies would directly strengthen the claims here: (1) a non-evaluative neutral-arm control to distinguish ``any history'' from ``any evaluative history''; (2) inter-rater agreement on the author-coded item categories with $\geq 2$ external annotators; and (3) the flipped-framing experiment (Section~\ref{sec:flipped}) extended across all 11 models to test whether the token-vs-semantic balance of the negativity asymmetry has a more robust per-model signature than the underpowered 2-model design here can detect.


\section{Conclusion}

AMEL shows up in every provider family I tested: 9 of 11 models are significant after Bonferroni. The overall magnitude is moderate ($d = -0.17$). What I find more troubling is where it concentrates---on the items where the model itself is uncertain at baseline, exactly the borderline cases where a judge most needs to be unbiased ($d = -0.34$ for high-entropy items). The asymmetry is real too: negative history pulls $1.62\times$ harder than positive in the paired test ($p < 10^{-39}$). And the shift is immediate. Five turns of biased history are enough; fifty do not deepen it.
...
The complete dataset (75,898 deduplicated API responses) is provided in JSONL format with deterministic seeds for all random operations.
...
Table~\ref{tab:model_detail} gives full results for all 11 models.
...
\bottomrule
\end{tabular}
\caption{Full results, all 11 models. $\bar{BS}$ = mean bias score, $d$ = Cohen's $d$, Sig.\ = significance after Bonferroni correction ($\alpha/21$). ***: $p_\text{corr} < 0.001$.}
...
Of the 75,898 deduplicated raw responses, 5,961 (7.85\%) failed to parse to a yes/no label.
A chi-squared test against the polarity factor rejects the null of independence ($\chi^2 = 97.7$, $df = 3$, $p < 10^{-20}$).
Per-polarity rates: baseline 10.77\%, no-saturated 6.96\%, yes-saturated 8.16\%, neutral 7.71\%.
...
Total dataset size goes from 78,084 raw rows to 75,898 deduplicated rows.
...
Across-paper headline numbers (overall $d = -0.17$, ambiguous $d = -0.28$, asymmetry $1.62\times$ paired) are unaffected by the dedup choice to the displayed precision.
