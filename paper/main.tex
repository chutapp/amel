...
Four external annotators recruited via Upwork (NLP / CS / research backgrounds, compensated at $\sim$\$40 / hour, full demographics released with the dataset) independently rated all 63 items into the same three categories, blind to the author labels and to one another. Inter-rater agreement is moderate overall (Krippendorff's $\alpha = 0.51$, Fleiss' $\kappa = 0.51$, full 4-way agreement on 47\% of items) and varies sharply by domain: meals $\alpha = 0.72$ (above the conventional $\alpha \geq 0.67$ NLP threshold), content moderation $\alpha = 0.54$, code review $\alpha = 0.28$. The lowest-agreement domain coincides with the most contested judgment (``is this code production-ready?''), which is itself consistent with this paper's core claim that ambiguous items are where the action is. For the per-category analyses in Section~\ref{sec:results} we use the majority-vote label across the four external annotators; items with no clear majority ($n = 7$) are excluded from per-category analyses but retained for overall, polarity, accumulation, contrast, and empirical-entropy analyses. Adjudication moved 8 items from ``ambiguous'' (author) to ``clear positive'' (majority); the headline statistics do not change under the relabelling (overall $d = -0.17$ unchanged; bootstrap 95\% CI $[-0.19, -0.15]$). The full per-annotator ratings, the codebook, and the agreement breakdown are released at \url{https://github.com/chutapp/amel/tree/main/data/annotators}.
...
The author thanks the four external annotators---annotator A, Marija Gavrilović, Ali, and Amna (consenting to be named)---for their independent ratings of the 63 test items used in the IRR study (Section~\ref{sec:domains}), and (volunteer, dropped from primary IRR) for a parallel exploratory pass kept separate from the headline statistics. All annotators consented to the public release of their pseudonymised ratings; pseudonymised data and the codebook are available at \url{https://github.com/chutapp/amel/tree/main/data/annotators}.
