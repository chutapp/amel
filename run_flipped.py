"""Flipped framing experiment (Phase 2).

Tests whether the negativity asymmetry follows the rejection *semantic frame*
or the 'no' *token*. Uses inverted question: 'Should this code be rejected?'

GPT-4.1 Nano + llama3.2:3b, 21 items, 3 conditions (baseline, no_sat@10, yes_sat@10).
10 reps → 630 calls per model → 1,260 total.
"""
...
from src.domains.code_review_flipped import CodeReviewFlippedDomain
from src.parser import parse_yes_no

DOMAIN = CodeReviewFlippedDomain()
POLARITIES_CTX = [
    ("baseline", 0),
    ("no_saturated", 10),
    ("yes_saturated", 10),
...
async def run_model(model_name, call_fn, concurrency, output_dir):
    ...
    conditions = []
    for polarity, ctx_len in POLARITIES_CTX:
        for item in test_items:
            for rep in range(REPETITIONS):
                key = f"{polarity}|{ctx_len}|{item.id}|{rep}"
                if key not in existing_keys:
                    seed = hash(f"{DOMAIN.name}|{polarity}|{ctx_len}|{item.id}|{rep}") & 0xFFFFFFFF
...
async def main():
    output_dir = Path("data/flipped")
    output_dir.mkdir(parents=True, exist_ok=True)

    # OpenAI
    if OPENAI_API_KEY:
        print(f"\n{'='*60}")
        print(f"Running flipped experiment: {OPENAI_MODEL}")
        print(f"{'='*60}")
    ...
    # Ollama
    print(f"\n{'='*60}")
    print(f"Running flipped experiment: {OLLAMA_MODEL}")
    print(f"{'='*60}")
    await run_model(OLLAMA_MODEL, call_ollama, OLLAMA_CONCURRENCY, output_dir)

    print(f"\nFlipped experiment complete! Results: {output_dir / 'results.jsonl'}")
